...
import { asyncForEach, viewport } from './e2e.utils';
...
const selectors = {
  root: '#jafar-react-layout-demos',
};

describe('Demos e2e', () => {
  it('Jafar react layout demos sanity', async () => {
...
async function testReactDemos(page) {
  await asyncForEach(menu.slice(1), async (parentItem) => {
    await testParentDemos(page, parentItem);
  });
}
...
async function testRouting(page) {
  const useCases = {
    createFormHtml: {
      url: 'item/layouts/html',
      selector: 'div#example-container > h2',
      expectedText: 'Item - Layouts',
    },
    createFormCode: {
      url: 'item/layouts/code',
      selector: 'div#example-code p',
      expectedText: 'Demo.jsx',
    },
  };
    // verify all docs
